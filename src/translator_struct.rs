...
Module that creates and measures single words
*/
...
use crate::reader::{
    AlliterationSettings, ConsonantStructureSettings, GeneralSettings, MiscSettings, StressSettings,
};
use crate::translator_ru::symbol_id;
use crate::translator_ru::ALL_VOWELS;
use crate::translator_ru::{transcript, Consonant, Vowel};
...
pub trait Voweable: Phonable {
    ...
    fn accent_dist(&self, second: &Self, sett: &StressSettings, index: usize) -> f32 {
        ...
        let k: f32 = match (self.accent(), second.accent()) {
            ...
            (A::NoAccent, A::Secondary)
            | (A::Secondary, A::NoAccent)
            | (A::Secondary, A::Secondary) => sett.k_not_strict_stress,
        ...
        k * self.distance(second, &sett.distance)
            / (index as f32 + sett.shift_syll_ending).powf(sett.pow_syll_ending)
    }
}
...
impl Word {
    pub fn new(w: &str, is_adj: bool) -> Self {
        ...
        for l in w.chars() {
            ...
            let new_current = {
                ...
                if Vowel::contains_char(&l) {
                    Phone::Vowel(Vowel {
                        letter: find_u8(l, Vowel::ALL.iter()),
                        accent: match l {
                            '!' => Accent::Primary,
                            _ => Accent::NoAccent,
                        },
                    })
                } else if Consonant::contains_char(&l) {
                    Phone::Consonant(Consonant {
                        letter: find_u8(l, Consonant::ALL.iter()),
                        voiced: false,
                        palatalized: false,
                    })
                } else {
            ...
            match new_current {
                Phone::None => match current {
                    ...
                    Phone::Consonant(ref mut c) => match l {
                        '*' => c.voiced = true,
                        '^' => c.palatalized = true,
    ...
    pub fn measure_vowel_dist(&self, other: &Self, sett: &StressSettings) -> f32 {
        ...
        for (i, (v1, v2)) in first_iter.zip(second_iterator).enumerate() {
            // self is smaller
    ...
    pub fn measure_cons_dist(&self, other: &Self, sett: &AlliterationSettings) -> f32 {
        ...
        for (syll_ind_1, (s_ind_1, len_1)) in other.splitted_consonants_rev().enumerate() {
            // iterating over first sylls
            for cons_ind_1 in s_ind_1..s_ind_1 + len_1 {
                ...
                for (syll_ind_2, (s_ind_2, len_2)) in self.splitted_consonants_rev().enumerate() {
                    // second sylls
        ...
        for (s_ind_1, len_1) in self.splitted_consonants_rev() {
            // iterating over first sylls
            for cons_ind_1 in s_ind_1..s_ind_1 + len_1 {
                // over first letters
                ...
                for (s_ind_2, len_2) in other.splitted_consonants_rev() {
                    // second sylls
                    for cons_ind_2 in s_ind_2..s_ind_2 + len_2 {
                        // second letter
                        let c2 =
    ...
    pub fn measure_struct_dist(&self, other: &Self, sett: &ConsonantStructureSettings) -> f32 {
        ...
        for (i, ((_, l1), (_, l2))) in self
            .splitted_consonants_rev()
            .zip(other.splitted_consonants_rev())
            .enumerate()
        {
            // self is smaller
            dist += (l1 as f32 - l2 as f32).abs().powf(sett.pow)
                / (i as f32 + sett.shift_syll_ending).powf(sett.pow_syll_ending);
        }
        dist / (self.vowel_count as f32 + sett.asympt_shift).powf(sett.asympt) * sett.weight
    }
    ...
    pub fn get_all_stresses(&self) -> Vec<(u8, usize)> {
        self.vowels()
            .rev()
            .enumerate()
            .filter_map(|(ind, vowel)| match vowel.accent {
